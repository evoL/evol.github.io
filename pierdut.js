...
(function() {
  var $, A1, A2, Attractor, Bounds, Formula, Grid, Params, RATIO, Reactor, Renderer, Size, StandardCurve, accelerationGrid, attractor, canvas, centerPoint, colorTimeout, correctionCurve, createCustomMapper, ctx, emulatedFormula, hex2rgb, hue2rgb, isRandom, pixelMapper, positionGrid, randomPoint, randomRange, randomizeAttractor, reactor, refreshingOperation, renderer, renderingEnabled, rgb2hsl, run, running, showState, size, toggle, updateBounds, updateBoundsSync, updateFormula, updateMapper, updateTTL, velocityGrid, verifyAttractor, viewBounds, viewCenterPoint, viewZoomLevel, zoomLevel;
  ...
  window.toggleRandomImpl = function() {
    randomPoint = isRandom ? function(n) {
      return {
        x: (0.5 + A1 * n) % 1,
        y: (0.5 + A2 * n) % 1
      };
  ...
  Reactor = (function() {
    ...
    Reactor.prototype.reset = function() {
      ...
      return this.system = (function() {
        ...
        for (i = j = 0, ref = this.count; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
          ...
          results.push({
            position: {
              x: point.x * this.bounds.width + this.bounds.left,
              y: point.y * this.bounds.height + this.bounds.top
            },
            velocity: {
              x: 0,
              y: 0
    ...
    Reactor.prototype.step = function() {
      var i, j, len, particle, point, position, ref, result, results, velocity;
      ref = this.system;
      results = [];
